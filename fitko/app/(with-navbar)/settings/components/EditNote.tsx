...
export default function EditNote(props: EditNoteProps) {
	...
	const title = useMemo(
		() => (props.type === "client" ? "Edit Client Description" : "Edit Session Description"),
		[props.type]
	);
	...
	useEffect(() => {
		...
		if (open) {
			setError(null);
			setDraft(props.initialNote ?? "");
			if (!dialog.open) dialog.showModal();
		} else {
			if (dialog.open) dialog.close();
		}
	}, [open, props.initialNote]);
						...
						<Button type="button" onClick={onSave} disabled={isPending}>
							{isPending ? (
								<span className="inline-flex items-center gap-2">
									<Loader2 className="w-4 h-3.5 animate-spin" />
									Saving…
								</span>
							) : (
								"Save"
